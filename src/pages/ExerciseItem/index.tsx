...
import { FETCH_EXERCISE_BY_ID_ENDPOINT, ADD_EXERCISE_FAVORITES_ENDPOINT, GET_FAVORITES_ENDPOINT } from '../../utils/constants/apiEndpoints';
      ...
      <div className='grid grid-cols-4 bg-black rounded h-screen -mt-5'>
        ...
        <div className='col-span-3 m-12 mt-16 flex flex-col border-1 rounded w-3/4 items-start h-52 p-4'>
          <div className='flex flex-row items-center w-full'>
            <p className='text-lg font-bold text-capital mr-3'>{exerciseData?.name}</p>
            <p className="bg-orange-400 rounded-full px-3 py-0.5 mx-2 my-1 text-capital">{exerciseData?.bodyPart}</p>
            <p className="bg-green-400 rounded-full px-3 py-0.5 mx-1 my-1 text-capital">{exerciseData?.target}</p>
            <div className='ml-auto flex flex-row items-center mr-3 hover:cursor-pointer' onClick={() => {isFav ? removeFavorite() : addFavorite()}}>
              <p className="mr-2">
                { isFav ? 
              ...
              <p className=''>{exerciseData?.favorites ? exerciseData?.favorites : 0}</p>
            </div>
          </div>
          <p className='text-gray-500 mb-6 text-capital'>{exerciseData?.equipment}</p>
          <p className='text-left'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequatur natus ex asperiores odio eum accusamus maxime deserunt, dolores tempore error repellat dolorem nam nihil a. Perspiciatis similique voluptate hic sapiente?</p>
        </div>
      </div>
  );
}
